...
<body>
    ...
    <script>
        ...
        function goToMainSession() {
            // Start the session and timer NOW when participant clicks Continue
            // This ensures timer synchronization - whichever team member clicks first starts the timer for both
        ...
        function checkApprovals() {
            ...
            // Update text area locking based on approvals
            updateTextAreaLock();
        ...
        function updateTextAreaLock() {
            ...
            if (anyApproved) {
                // Lock both fields when ANY approval is checked
                ...
                // Unlock when BOTH approvals are unchecked
...
def get_typing_metrics(team_id, participant_id):
    """Get typing metrics for a participant to restore after refresh"""
    if not team_id or not participant_id:
        return jsonify({'error': 'Team ID and Participant ID required'}), 400
    ...
    # Get metrics for this participant
    title_keystroke_col = f'p{participant_id}_title_keystroke_count'
    title_seconds_col = f'p{participant_id}_title_active_typing_seconds'
    title_first_col = f'p{participant_id}_title_first_edit_time'
    title_last_col = f'p{participant_id}_title_last_edit_time'
    desc_keystroke_col = f'p{participant_id}_description_keystroke_count'
    desc_seconds_col = f'p{participant_id}_description_active_typing_seconds'
    desc_first_col = f'p{participant_id}_description_first_edit_time'
    desc_last_col = f'p{participant_id}_description_last_edit_time'

    c.execute(f'''
        SELECT {title_keystroke_col}, {title_seconds_col}, {title_first_col}, {title_last_col},
               {desc_keystroke_col}, {desc_seconds_col}, {desc_first_col}, {desc_last_col}
        FROM teams WHERE team_id = ?
    ''', (team_id,))
    row = c.fetchone()
    conn.close()
        ...
        for participant_id in ['1', '2']:
            ...
            if tracked_stage:
                ...
                if survey_page3_done:
                    ...
                    # Check if there's any heartbeat - means they logged in at least
                    if participant_heartbeat:
                        stage = 'comprehension'
            ...
            if stage:
                participant_key = 'p1' if participant_id == '1' else 'p2'
                team_data[participant_key] = {
                    'stage': stage,
                    'last_update': last_update
                }

        # Only add team if at least one participant has data
